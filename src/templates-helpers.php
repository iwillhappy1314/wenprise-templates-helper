 ...
 * @since 4.3.0
 */
function wenprise_tokenize_path($path, $path_tokens)
{
 ...
 * @since 4.3.0
 */
function wenprise_untokenize_path($path, $path_tokens)
{
    foreach ($path_tokens as $token => $token_path) {
 ...
 */
function wenprise_get_path_define_tokens()
{
    $defines = [
        'ABSPATH',
        'WP_CONTENT_DIR',
 ...
 * @since 4.3.0
 */
function wenprise_set_template_cache($cache_key, $template)
{
 ...
 * @since 4.3.0
 */
function wenprise_clear_template_cache()
{
    $cached_templates = wp_cache_get('cached_templates', 'wenprise');
    if (is_array($cached_templates)) {
 ...
 * @return string
 */
function wenprise_locate_template($template_name, $template_path = '', $default_path = '')
{
    if ( ! $template_path) {
        $template_path = 'wenprise/';
    }

    if ( ! $default_path) {
        $default_path = WENPRISE_ELEMENTOR_DIR . '/templates/';
    }
 ...
 * @param string $default_path  Default path. (default: '').
 */
function wenprise_get_template($template_name, $args = [], $template_path = '', $default_path = '')
{
    $cache_key = sanitize_key(implode('-', ['template', $template_name, $template_path, $default_path, WENPRISE_ELEMENTOR_VERSION]));
    $template  = (string)wp_cache_get($cache_key, 'wenprise');

    if ( ! $template) {
        $template = wenprise_locate_template($template_name, $template_path, $default_path);

        // Don't cache the absolute path so that it can be shared between web servers with different paths.
        $cache_path = wenprise_tokenize_path($template, wenprise_get_path_define_tokens());

        wenprise_set_template_cache($cache_key, $cache_path);
    } else {
        // Make sure that the absolute path to the template is resolved.
        $template = wenprise_untokenize_path($template, wenprise_get_path_define_tokens());
    }
